...
from abdiff.abdiff_sampling.nn.AF2.tensor_utils import permute_final_dims
...
class TriangleMultiplicativeUpdate(BaseTriangleMultiplicativeUpdate):
    ...
    Implements Algorithms 11 and 12.
    """
    def __init__(self, c_z, c_hidden, _outgoing=True, norm_type='adanorm'):
        """
        Args:
            c_z:
                Input channel dimension
        ...
        if norm_type == 'adanorm':
            self.layer_norm_in = AdaLayerNorm(self.c_z, config.num_train_timesteps)
            self.layer_norm_out = AdaLayerNorm(self.c_hidden, config.num_train_timesteps)
    ...
    def forward(self, 
        ...
        if self.norm_type == 'adanorm':
            z = self.layer_norm_in(z, timestep)
        else:
        ...
        del a, b
        if self.norm_type == 'adanorm':
            x = self.layer_norm_out(x, timestep)
        else:
            x = self.layer_norm_out(x)
        x = self.linear_z(x)
